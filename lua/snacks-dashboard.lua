...
▀ ██  ██   ██   ██     ██▄▄▄▄ ██   ██ ██    ██ ██ ██ ▄▄▄▄▄]],
  ...
  sections = (function()
    ...
    if vim.fn.executable("cbonsai") == 1 then
      ...
      -- WHY: Create visually interesting terminal output with what the user has installed
      ...
      if has_fortune or has_cowsay or has_bat or has_lolcat then
        ...
        local cmd = has_fortune and "fortune -s" or "echo \"Thank you for using HYPERfix.nvim!\""
        ...
        table.insert(sections, { section = "terminal", cmd = cmd, height = 12 })
      end
      -- If none of the above packages are available, skip terminal section entirely
    end

    -- Core dashboard sections (always included)
    table.insert(sections, { section = "header", padding = 1 })
    table.insert(sections, { section = "keys", padding = 1 })
    table.insert(sections, { icon = " ", title = "Recent Files", section = "recent_files", indent = 2, padding = 1 })
    table.insert(sections, { icon = " ", title = "Projects", section = "projects", indent = 2, padding = 1 })
